...
namespace UmamusumeResponseAnalyzer.AI
{
    ...
    public class GameStatusSend_Ura
    {
        ...
        //SupportCard cardParam[6];//六张卡的参数，拷贝到Game类里，一整局内不变，顺序任意。这样做的目的是训练ai时可能要随机改变卡的参数提高鲁棒性，所以每个game的卡的参数可能不一样
        //int16_t saihou;//赛后加成
        public UraPerson[] persons;//如果不带其他友人团队卡，最多19个头。依次是15个可充电人头（先是支援卡（顺序随意）：0~4或5，再是npc：5或6~14），理事长15，记者16，桐生院17（带没带卡都是17），绿帽18
        //bool isRacing;//这个回合是否在比赛
        ...
        public GameStatusSend_Ura(Gallop.SingleModeCheckEventResponse @event)
        {
...

            {
                ...
                for (int i = 0; i < 6; i++)
                {
                    ...
                    {
                        ura_lmType = 1;
                        persons[18].cardIdInGame = i;
                        headIdConvert[i + 1] = 18;
            ...
            if (ura_lmType != 0)
            {
                ...
                var d = @event.data.chara_info.evaluation_info_array.First(x => x.target_id == headIdConvert.First(x => x.Value == 18).Key);
            ...
            if (ura_tsyType == 0)
            {
                headIdConvert[104] = 17;
            }
            headIdConvert[101] = 18;
